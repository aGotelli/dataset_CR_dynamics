function [N_disks, timestamps, poses_disks, rel_poses_disks, rel_kinematics_disks] = data_optitrack(filename)
    ...
    for it = 1:N_disks
        ...
        % rel_pose_disk = pagemtimes(rel_transf_mocap, pose_disk);
    ...
    if make_video
        ...
        for it_t = 1:N_time
            ...
            % z_ref = R_ref(:, 3);
            % 
            % quiver3(r_ref(1), r_ref(2), r_ref(3), x_ref(1)*2*axis_len, x_ref(2)*2*axis_len, x_ref(3)*2*axis_len, 'r', 'LineWidth', 4, 'AutoScale', 'off');
