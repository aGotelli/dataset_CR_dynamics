function sync_results = check_temporal_sync(time_mot, angles, ...
        time_moc, kin_disks, time_fbg, fbgs_shapes, FBGS_tip_index, saving_folder)
...
% close all;
% 
% fs_mot = 1 / median(diff(time_mot));
% ml     = round(0.25 * fs_mot);          % ±250 ms window
% 
% 
% angles_nomralized = angles - mean(angles);
% 
...
% 
% max_lag = lags13(find(c13 == max(c13)))/fs_mot * 1000;
% 
% figure(1)
% subplot(2, 1, 1)
% plot(time_mot, angles(:, 1), 'b');
% hold on
% plot(time_mot, -angles(:, 3), 'r');
% grid on
% 
% subplot(2, 1, 2)
% plot(time_mot, angles(:, 2), 'b');
% hold on
% plot(time_mot, -angles(:, 4), 'r');
% grid on
% 
% figure(2)
% subplot(2, 1, 1)
...
% 
% 
% tip_mocap = kin_disks(:,4:6,5);
% tip_mocap_normalized = tip_mocap - mean(tip_mocap);
% figure()
% plot(time_moc, tip_mocap(:, 1), 'r');
% hold on;
% plot(time_moc, tip_mocap(:, 2), 'g');
% plot(time_moc, tip_mocap(:, 3), 'b');
% 
% yyaxis right
% plot(time_mot, angles(:, 1), 'c');
% hold on
% plot(time_mot, -angles(:, 3), '--c');
% 
% plot(time_mot, angles(:, 2), 'w');
% plot(time_mot, -angles(:, 4), '--w');
% 
% grid on
% 
% 
% 
% [c1_x, lags1_x] = xcorr(angles(:, 1), tip_mocap(:, 1));
% [c1_y, lags1_y] = xcorr(angles(:, 1), tip_mocap(:, 2));
% [c1_z, lags1_z] = xcorr(angles(:, 1), tip_mocap(:, 3));
% 
% [c3_x, lags3_x] = xcorr(angles(:, 1), tip_mocap(:, 1));
% [c3_y, lags3_y] = xcorr(angles(:, 1), tip_mocap(:, 2));
% [c3_z, lags3_z] = xcorr(angles(:, 1), tip_mocap(:, 3));
% 
% figure()
% subplot(2, 1, 1)
% stem(lags1_x, c1_x)
...
% 
% 
% fs_moc = 1 / median(diff(time_moc));
% 
% % Motor vs mocap (different lengths -> manual normalization)
% a = angles_nomralized(:,1);
% b = tip_mocap_normalized(:,1);
% [c, lags] = xcorr(a, b, ml);
% c = c / sqrt(sum(a.^2) * sum(b.^2));
% [~, idx] = max(abs(c));
% lag_MO_ms = lags(idx) / fs_moc * 1000
% r_MO      = c(idx)
% 
% 
% 
...
% 
% %% Output
% sync_results.lag_MO      = lag_MO;           % [2x3] agonist motor -> OptiTrack [ms]
% sync_results.r_MO        = r_MO;
% sync_results.lag_MF      = lag_MF;           % [2x3] agonist motor -> FBGS [ms]
% sync_results.r_MF        = r_MF;
% sync_results.lag_OF      = lag_OF;           % [1x3] OptiTrack -> FBGS per axis [ms]
% sync_results.r_OF        = r_OF;
% sync_results.lag_OF_mean = mean(lag_OF);     % scalar: mean pipeline delay [ms]
% sync_results.lag_OF_std  = std(lag_OF);      % scalar: spread across axes [ms]
% sync_results.fbg_delay_s = -mean(lag_OF) / 1000;  % correction to apply in process_data [s]
% sync_results.lag_M13     = lag_M13;          % M(+x) leads M(-x) [ms]
% sync_results.r_M13       = r_M13;
% sync_results.lag_M24     = lag_M24;          % M(+y) leads M(-y) [ms]
% sync_results.r_M24       = r_M24;
% sync_results.fs_mot      = fs_mot;           % effective motor rate [Hz]
% sync_results.fs_moc      = fs_moc;           % effective mocap rate [Hz]
% 
% end
% 
% 
