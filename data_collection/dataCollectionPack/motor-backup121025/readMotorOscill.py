...
parser.add_argument("--motor2-id", type=int, default=3, help="CAN ID for antagonistic motor (default: 3)")
parser.add_argument("--radius", type=float, default=20.0, help="Radius of oscillation in degrees")
...
filename = f"{name}_oscillation_radius_{suffix}{ext}"

print(
    f"Motors {args.motor1_id} & {args.motor2_id}: oscillation with radius ±{args.radius:.1f}° "
    f"over {duration:.2f}s, logging to {filename}"
)
...
while status1 is None and status2 is None and attempts < 20:
    ...
    if s1[1] is not None and s2[1] is not None:
        status1 = s1
        status2 = s2
        print(f"Status s1:  {s1}")
        print(f"Status s2:  {s2}")
        break
    attempts += 1
...
home1 = status1[1]
home2 = status2[1]
print(f"Home s1:  {home1}")
print(f"Home s2:  {home2}")
# Define the two alternating phases for oscillation
phase_targets = [
        (home1 - ramp_rad, home2 + ramp_rad),
        (home1 + ramp_rad, home2 - ramp_rad),
        ]

samples = []
current_phase = 0  # Start with phase A

# Main oscillation loop - single loop structure  
command_sent = False
phase_start = time.time()

while time.time() - start_time < duration:
    now = time.time()
    target1, target2 = phase_targets[current_phase]
    print(f"ramp: {ramp_rad} - home1: {home1} - Target 1: {target1} ; home2: {home2} - target2: {target2}")
    
    # Send command to motors only once per phase
    if not command_sent:
        ...
        command_sent = True
        phase_start = now
        # print(f"Phase {current_phase}: HOME1: {math.degrees(home1):.1f}° target1: {math.degrees(target1):.1f}° -- HOME2: {math.degrees(home2):.1f}° target2: {math.degrees(target2):.1f}°")
    ...
    if finish1:
        print(f"END1!!!!!!!!!! {abs_angle_rad1} vs {target1}")
    if finish2:
        print(f"END2!!!!!!!!!! {abs_angle_rad2} vs {target2}")

    if finish1 and finish2:
        print(f"Phase {current_phase} completed - switching to next phase")
        # Switch to the other phase for continuous oscillation
        current_phase = 1 - current_phase  # Toggle between 0 and 1
        command_sent = False  # Reset flag for next phase
