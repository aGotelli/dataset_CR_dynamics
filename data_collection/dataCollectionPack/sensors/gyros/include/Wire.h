...
public:
#ifdef _WIN32
    TwoWire(const uint8_t device = 0) : deviceNum(device) { std::cout << "TwoWire constructor called.\n"; }
    ...
    void begin()
    {
        if (!CH341OpenDevice(deviceNum))
        {
            std::cerr << "Failed to open CH341 device deviceNum" << std::endl;
            return;
        }

        if (!CH341SetStream(deviceNum, 3))
        { // Set I2C mode
            std::cerr << "Failed to set I2C mode" << std::endl;
            CH341CloseDevice(deviceNum);
            return;
        }
    ...
    int endTransmission(bool stop = true)
    {
        ...
        else if (txBuffer.size() >= 2)
        {
            ...
            {
                success = success & CH341WriteI2C(deviceNum, targetAddress, regAddr, item);
            }
            if (!success)
            {
        ...
        { // Buffer is empty
            ...
            bool success = CH341WriteI2C(deviceNum, targetAddress, 0x00, 0x00);
            return success ? 0 : -1;
        }
    ...
    uint16_t requestFrom(uint8_t address, uint8_t numBytes, bool stop = true)
    {
        ...
        {
            uint8_t read_byte;
            success = success & CH341ReadI2C(deviceNum, address, lastRegisterAddress, &read_byte);
    ...
    {
        return std::to_string(deviceNum); 
    }

private:
    const uint16_t deviceNum;
