...
set output_dir=reachPT_m160-m2-40
...
@REM start "ATI FT" cmd /k "py readATIFT.py %duration% dataATIFT%postfix%.csv --start-time %start_time%"
...
REM ********************************    GYROSCOPE      ********************************
@REM @REM start "Gyro" cmd /k ".\sensors\gyros\build\Debug\sparkfun_ism330dhcx.exe dataCollectionPack/gyros 300 %duration%"
...
start "PontReach" cmd /k "py read4motorToPoint.py %duration% %output_dir%\dataMotor%postfix%.csv --motor1-id 1 --motor2-id 2 --motor3-id 3 --motor4-id 60 --increment-deg1 40 --increment-deg2 -40 --start-time %start_time%"
...
@REM start "Four Motor Circle" cmd /k "py read4MotorCircle.py %duration% %output_dir%\datasequence%postfix%.csv --motor1-id 1 --motor2-id 2 --motor3-id 3 --motor4-id 4 --radius 60 --start-time %start_time%"
