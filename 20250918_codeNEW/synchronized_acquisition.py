...
class SynchronizedDataAcquisition:
    ...
    def _mark10_acquisition_wrapper(self, sensor, sensor_number, duration, output_file):
        """Wrapper for Mark-10 acquisition with detailed error reporting"""
        try:
            print(f"📊 Mark-10 sensor {sensor_number}: Starting acquisition...")
            print(f"    Port: {sensor.com_port}")
            print(f"    Output: {output_file}")
            
            success = sensor.acquire_data(duration, output_file)
            
            if success:
                print(f"✅ Mark-10 sensor {sensor_number}: Acquisition completed successfully")
                # Check if file was actually created
                if os.path.exists(output_file):
                    file_size = os.path.getsize(output_file)
                    print(f"    File created: {file_size} bytes")
                else:
                    print(f"❌ Mark-10 sensor {sensor_number}: File was not created!")
            else:
                print(f"❌ Mark-10 sensor {sensor_number}: Acquisition failed")
                
        except Exception as e:
            print(f"❌ Mark-10 sensor {sensor_number}: Exception during acquisition: {e}")
            import traceback
            traceback.print_exc()
    ...
    def run_tension_controlled_acquisition(self, experiment_name, duration, additional_info=""):
        ...
        for i, sensor in enumerate(self.mark10_sensors):
            mark10_thread = threading.Thread(
                target=self._mark10_acquisition_wrapper,
                args=(sensor, i+1, duration, os.path.join(self.experiment_dir, f"mark10_{i+1}.csv"))
            )
            mark10_thread.start()
            self.threads.append(mark10_thread)
