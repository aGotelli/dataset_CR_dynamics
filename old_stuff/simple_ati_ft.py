...
# Easy-to-use functions for different scenarios

def quick_test(duration=5):
    """Quick test with default settings"""
    print("=== Quick ATI F/T Sensor Test ===")
    sensor = SimpleATI_FT()
    return sensor.acquire_data(duration, "quick_test.csv")

def standard_acquisition(duration=10, rate=1000, output="ati_data.csv"):
    """Standard data acquisition"""
    print("=== Standard ATI F/T Data Acquisition ===")
    sensor = SimpleATI_FT(sampling_rate=rate)
    return sensor.acquire_data(duration, output)

def high_speed_acquisition(duration=30, rate=2000, output="ati_highspeed.csv"):
    """High-speed data acquisition"""
    print("=== High-Speed ATI F/T Data Acquisition ===")
    sensor = SimpleATI_FT(sampling_rate=rate)
    return sensor.acquire_data(duration, output)

def no_bias_acquisition(duration=10, output="ati_nobias.csv"):
    """Acquisition without bias calculation (raw data)"""
    print("=== Raw ATI F/T Data Acquisition (No Bias) ===")
    sensor = SimpleATI_FT()
    return sensor.acquire_data(duration, output, calculate_bias=False)

def raw_voltages_acquisition(duration=10, output="ati_with_voltages.csv"):
    """Acquisition with both raw voltages and F/T data"""
    print("=== ATI F/T Data with Raw Voltages ===")
    sensor = SimpleATI_FT()
    return sensor.acquire_data(duration, output, calculate_bias=True, save_raw_voltages=True)

def postprocess_ready_acquisition(duration=10, output="ati_for_postprocess.csv"):
    """Acquisition optimized for postprocessing - includes raw voltages, no bias"""
    print("=== Data Acquisition for Postprocessing ===")
    print("Saving raw voltages + F/T values (no bias correction)")
    sensor = SimpleATI_FT()
    return sensor.acquire_data(duration, output, calculate_bias=False, save_raw_voltages=True)
