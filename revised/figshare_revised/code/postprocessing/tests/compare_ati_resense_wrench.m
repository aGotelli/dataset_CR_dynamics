...
for ir = 1:numel(recordings)
    ...
    required_files = ["base_wrench.csv", "contact_wrench.csv", "mocap_frames.csv", "contact_pose.csv"];
    ...
    for it = 1:2
        subplot(2, 1, it)
        plot(sampling_time, interp_base_wrench(:, it), 'b', 'LineWidth', lineWidth)
        hold on
        plot(sampling_time, wrench_at_base(it, :), 'r', 'LineWidth', lineWidth)
    ...
    for it = 1:3
        subplot(3, 1, it)
        plot(sampling_time, interp_base_wrench(:, 3 + it), 'b', 'LineWidth', lineWidth)
        hold on
        plot(sampling_time, wrench_at_base(3 + it, :), 'r', 'LineWidth', lineWidth)
    ...
    for it = 1:3
        subplot(3, 1, it)
        plot(sampling_time, interp_rel_kinematics_disks_corr(:, 3 + it, tip_disk_index), 'b', 'LineWidth', 1, 'LineWidth', lineWidth)
        hold on
        plot(sampling_time, pos_sensor(it, :), 'r', 'LineWidth', 1, 'LineWidth', lineWidth)
    ...
    plot(sampling_time, dist_contact, 'b', 'LineWidth', lineWidth)
    hold on
    yline(contact_threshold, 'r--', 'LineWidth', lineWidth)
    ylabel('Distance [m]')
