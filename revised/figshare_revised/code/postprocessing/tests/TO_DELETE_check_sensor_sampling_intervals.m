...
function one_row = make_detail_row(subset_name, recording_name, sensor_label, gaps_in_seconds)
    ...
    % Coefficient of variation: the standard, unit-free measure of how
    % regular the sampling actually is. Near 0 means uniform; larger
    % means jittery. Unlike MeanOverMedianRatio, this does not rely on
    % mean and median disagreeing -- it is sensitive to jitter even when
    % the gap distribution is symmetric (mean == median).
    cv_percent = 100 * std_gap_ms / mean_gap_ms;

    % Worst-case gap relative to the typical (median) gap. Large values
    % flag a rare-but-severe outlier/dropout pattern (e.g. Mark-10's
    % multi-hundred-ms stalls) that a small CV can still hide when the
    % outlier is rare enough not to move the standard deviation much.
    max_over_median = max_gap_ms / median_gap_ms;

    % Fraction of gaps more than 2x the median gap: a direct, literal
    % "how often does a burst/dropout actually happen" count, separate
    % from how bad the single worst one is.
    pct_gaps_above_2x_median = 100 * sum(gaps_ms > 2*median_gap_ms) / numel(gaps_ms);
...
function one_row = make_summary_row(sensor_name, pooled_gaps_in_seconds, number_of_recordings)
    ...
    % Coefficient of variation: the standard, unit-free regularity
    % metric. Near 0 means uniform sampling; larger means jitter. Unlike
    % MeanOverMedianRatio, it does not require mean and median to
    % disagree -- symmetric jitter with mean == median still shows up
    % here.
    cv_percent = 100 * std_gap_ms / mean_gap_ms;

    % Worst-case gap relative to the typical (median) gap: flags a
    % rare-but-severe outlier/dropout pattern that CV can under-report
    % when the outlier is rare enough not to move the standard deviation
    % much (e.g. Mark-10's multi-hundred-ms stalls against a ~2.8 ms
    % median).
    max_over_median = max_gap_ms / median_gap_ms;

    % Fraction of gaps more than 2x the median gap: how OFTEN a
    % burst/dropout happens, as distinct from how bad the single worst
    % one is (MaxOverMedianRatio).
    pct_gaps_above_2x_median = 100 * sum(gaps_ms > 2*median_gap_ms) / numel(gaps_ms);
