...
% does not check it.
%
% This script calls data_optitrack.m on every recording, and counts how 
% many valid and invalid frames come back for every disk of every recording. 
% Whenever a disk was ever occluded, it also reports exactly which 
% recording, disk, and sample, so the finding can be traced back to the 
% raw file.
%
% OUTPUT
% ------
...
% (mfilename('fullpath')) rather than a "../../..."-style relative path,
% so it resolves correctly regardless of MATLAB's current folder. This
% script lives at .../code/postprocessing/tests/.
...
for subset_index = 1:numel(subset_names)
    ...
    for recording_index = 1:numel(recording_folders)
        ...
        % has a dataResenseFT.csv file, rather than assumed from the
        % subset name.
        ...
        % except also requesting its sixth, optional output:
        ...
        for disk_index = 1:N_disks
            ...
            % while this loop counts disk_index from 1. disk_number
            % converts back to the 0-indexed name so the reported disk
            % number matches the CSV column.
