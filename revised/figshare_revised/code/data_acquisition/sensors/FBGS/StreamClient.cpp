...
int StreamClient::readStream()
{
    ...
    //                shape[0], shape[1], shape[2], shape[3], shape[4], shape[5], shape[6], shape[7], shape[8], shape[9], shape[10], shape[11]);
        ...
        std::vector<float> cur, ang, shp;
        if (curvatureArrayLength > 0 && curvature != nullptr) cur.assign(curvature, curvature + curvatureArrayLength);
        if (angleArrayLength > 0 && angle != nullptr) ang.assign(angle, angle + angleArrayLength);
        if (shapeArrayLength > 0 && shape != nullptr) shp.assign(shape, shape + shapeArrayLength);
        dataToSave.emplace_back(machine_timestamp, std::move(cur), std::move(ang), std::move(shp));
